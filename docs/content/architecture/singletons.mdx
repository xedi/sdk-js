...
<Flash>
    <InfoIcon mr={1} />Don't confuse `resolveInstance` and `resolve`. `resolveInstance` is a state method which can be used to resolve the active container for static methods.
