...
Within the `docs/` directory, there are a number of important directories and files to be aware of. You probably already know about the `package.json` and `.env` file. If not, read the <Link to="/documentation/set-up">Getting Started</Link> section.
