...
<Flash mb={2}><InfoIcon /> Services is perhaps the wrong naming convention here; think instead entities.</Flash>
...
Lets look at line three of the above example. In JavaScript this would look like `.get(`1/examples`)`, so whats the extra stuff? This how we tell TypeScript, what we expect the <Link href="architecture/http_requests">HTTP Client</Link> to return in the promise. Here we tell it we want a `JsonResponse` which itself, most contain an `Example`.
